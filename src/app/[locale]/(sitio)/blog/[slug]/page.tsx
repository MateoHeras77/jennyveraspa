...
export async function generateMetadata({ params }: BlogPostPageProps): Promise<Metadata> {
  ...
  if (!post) {
    return {
      title: locale === "es" ? "Articulo no encontrado" : "Article not found",
      robots: {
        index: false,
        follow: false,
      },
  ...
  const alternates = getLocaleAlternates(locale, `/blog/${post.slug}`);
...
export default async function LocalizedBlogPostPage({ params }: BlogPostPageProps) {
  ...
  if (!isValidLocale(locale)) {
    redirect(withLocalePath("es", `/blog/${slug}`));
  }
  ...
  const relatedPosts = await getRelatedPostsByTags(slug, 3, locale);
  const headings = getHeadingsFromMarkdown(post.body);
  const canonical = getLocaleAlternates(locale, `/blog/${post.slug}`).canonical;

  const breadcrumbs = [
    {
      name: locale === "es" ? "Inicio" : "Home",
      item: toSiteAbsoluteUrl(withLocalePath(locale, "/")),
    },
    {
      name: "Blog",
      item: toSiteAbsoluteUrl(withLocalePath(locale, "/blog")),
    },
    {
      name: post.title,
      item: canonical,
    ...
    <div className="min-h-screen bg-[linear-gradient(180deg,#141414_0%,#141414_23rem,#FDFBF7_23rem,#FDFBF7_100%)] pb-20 pt-28">
      <article className="container mx-auto max-w-4xl px-6">
        ...
        <nav className="mt-10 grid gap-4 md:grid-cols-2">
          ...
          {next ? (
            <Link
              href={withLocalePath(locale, next.href)}
            ...
            >
              <p className="text-[11px] uppercase tracking-[0.16em] text-gray-500">
                {locale === "es" ? "Articulo siguiente" : "Next article"}
              </p>
        ...
        {relatedPosts.length > 0 ? (
          <section className="mt-16">
            <div className="mb-7 flex items-end justify-between gap-4">
              ...
              <Link
                href={withLocalePath(locale, "/blog")}
                className="text-xs uppercase tracking-[0.16em] text-gray-600 hover:text-[#9A7A1F]"
              >
